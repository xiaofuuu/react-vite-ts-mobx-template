...
import { ShowLoading, HideLoading } from '@/components/GlobLoading'

const DefauldLoading = true
...
export class LYRequest {
  ...
  constructor(config: LYRequestConfig) {
    ...
    this.showLoading = config.showLoading ?? DefauldLoading
  ...
  request<T>(config: LYRequestConfig<T>): Promise<T> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.instance
        .request<any, T>(config) // 2,改为T类型后就需要来这里修改泛型 原本T的位置是AxiosResponse 相应的 应该去到type.d.ts文件将对应的类型修改 为了不报错 可以改为any类型 详见type.d.ts文件
        .then((res) => {
          // 请求完后设置响应拦截
          ...
          this.showLoading = DefauldLoading
          resolve(res)
        })
        .catch((err) => {
          this.showLoading = DefauldLoading
